...
if not args.save_optimizer:
    saver_to_save = tf.train.Saver(max_to_keep=1000)
    saver_best = tf.train.Saver(max_to_keep=1000)

# 放到这里
tf.contrib.quantize.create_training_graph(
    input_graph=tf.get_default_graph(),weight_bits=8,bias_bits=8,
    activation_bits=8,quant_delay=40000) # quant_delay是指多少次迭代之后再进行量化，如果是已训练好进行微调量化的话，那么可以将quant_del ay设为0。
...
with tf.Session() as sess:
    ...
    '''ckpt = tf.train.get_checkpoint_state('/home/zhangchi/yolov3ww/checkpoint/')
    ...
    for epoch in range(args.total_epoches):
        ...
        if __global_step % args.train_evaluation_step == 0 and __global_step > 0:
            ...
            recall, precision = evaluate_on_gpu(sess, gpu_nms_op, pred_boxes_flag, pred_scores_flag, __y_pred, __y_true,
                                                args.class_num, args.nms_threshold)
        ...
        if epoch % args.val_evaluation_epoch == 0 and epoch >= args.warm_up_epoch:
            ...
            if mAP > best_mAP:
                best_mAP = mAP
                saver_best.save(sess, args.save_dir + 'best_model-epoch_{}_step_{}_mAP_{:4f}_loss_{:.4f}_lr_{:.5g}'.format(
                    epoch,int(__global_step),mAP,val_loss_total.average,__lr), global_step=int(__global_step))
