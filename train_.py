    ...
    for i in net_def_list:
        if i['operation']=="add":
            ...
            for name in i['previous_layer']:
                ignore_set.add(name[0:-12])
gamme_vars = []
for item in update_vars:
    if "gamma" in item.name:
        if item.name[0:-18].replace('/','_') in ignore_set:
            pass
        else:
            #后续是add层，concat层无需正则，可以不添加到gamme_vars中，后续可以改一下
            gamme_vars.append(item)
...
with tf.control_dependencies(update_ops):
    ...
    gradient = optimizer.compute_gradients(loss[0], var_list = update_vars)
    gamma_gradient = optimizer.compute_gradients(loss[0], var_list = gamme_vars)
    clip_grad_var = [gv if gv[0] is None else [tf.clip_by_norm(gv[0], 100.), gv[1]] for gv in gradient]
    # L1 Loss
    gradient_with_l1 = [gv if gv[0] is None else [0.01 * tf.sign(gv[1]), gv[1]] for gv in gamma_gradient] + clip_grad_var
    # train_op = optimizer.apply_gradients(clip_grad_var, global_step=global_step)
    train_op_with_l1 = optimizer.apply_gradients(gradient_with_l1, global_step=global_step)

if args.save_optimizer:

    print('Saving optimizer parameters to checkpoint! Remember to restore the global_step in the fine-tuning afterwards.')
    saver_to_save = tf.train.Saver(max_to_keep=10000)
    saver_best = tf.train.Saver(max_to_keep=10000)
...
with tf.Session() as sess:
    ...
    for epoch in range(args.total_epoches):
        ...
        # print(sess.run("yolov3/darknet53_body/Conv/BatchNorm/gamma:0"))
        ...
        for i in trange(args.train_batch_num):
            ...
            _, summary, __y_pred, __y_true, __loss, __global_step, __lr = sess.run(
                [train_op_with_l1, merged, y_pred, y_true, loss, global_step, learning_rate],
        ...
        if epoch % args.val_evaluation_epoch == 0 and epoch >= args.warm_up_epoch:
            ...
            if mAP > best_mAP:
                ...
                saver_best.save(sess, args.save_dir + 'best_model_Epoch_{}_step_{}_mAP_{:.4f}_loss_{:.4f}_lr_{:.7g}'.format(
                                ...
                                epoch, int(__global_step), mAP, val_loss_total.average, __lr))
