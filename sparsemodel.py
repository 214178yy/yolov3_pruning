...
from utils.sparse_layer_utils import conv2d, darknet53_body, yolo_block, upsample_layer


def conv2dcheck(inputs, filters, kernel_size, strides=1, is_training=True):
    ...
    inputs = slim.conv2d(inputs,filters,
                         ...
                         padding=('SAME' if strides == 1 else 'VALID') , scope='featconcheck'  )
    return inputs
...
class yolov3(object):
    ...
    def forward(self, inputs, is_training=True, reuse=False):
        ...
        with tf.variable_scope('darknet53_body'):
            route_1, route_2, route_3 = darknet53_body(inputs, is_training=is_training)

        f=open("net_channel.json","r")
        channel_list = json.loads(f.read())
        f.close()


        with tf.variable_scope('yolov3_head'):
            # inter1, net = yolo_block(route_3, 512,is_training=is_training )
            net = conv2d(route_3, channel_list["yolov3_yolov3_head_Conv"], 1, is_training=is_training)  # 0
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_1"], 3, is_training=is_training)  # 1
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_2"], 1, is_training=is_training)  # 2
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_3"], 3, is_training=is_training)  # 3
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_4"], 1, is_training=is_training)  # 4
            inter1 = net
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_5"], 3, is_training=is_training)  # 5
            ...
            feature_map_1 = slim.conv2d(net,
                                        ...
                                        weights_initializer=weights_initializer,
                                        weights_regularizer=weights_regularizer,
                                        # scope='feature_map_001'
                                        )
            feature_map_1 = tf.identity(feature_map_1, name='feature_map_001')

           # inter1 = conv2dcheck(inter1, 256, 1)

            activation_fncheck = tf.nn.relu6
            # activation_fncheck = tf.nn.relu
            normalizer_fncheck = tf.contrib.slim.batch_norm
            normalizer_paramscheck = {
                'is_training': is_training,
                'center': True,
                'scale': True,
                'decay': 0.9997,
            ...
            weights_initializercheck = tf.truncated_normal_initializer(stddev=0.09)
            weights_regularizercheck = tf.contrib.layers.l2_regularizer(0.00004)
            inter1 = slim.conv2d(inter1,256, 1,
                                        activation_fn=activation_fncheck,
                                        normalizer_fn=normalizer_fncheck,
                                        normalizer_params=normalizer_paramscheck,
                                        weights_initializer=weights_initializercheck,
                                        weights_regularizer=weights_regularizercheck,
                                        # scope='feature_map_check'
                                        )
            ...
            net = conv2d(concat1, channel_list["yolov3_yolov3_head_Conv_8"], 1, is_training=is_training)  # 8
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_9"], 3, is_training=is_training)  # 9
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_10"], 1, is_training=is_training)  # 10
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_11"], 3, is_training=is_training)  # 11
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_12"], 1, is_training=is_training)  # 12
            inter2 = net
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_13"], 3, is_training=is_training)  # 13

            activation_fn2 = tf.nn.relu6
            # activation_fn2 = tf.nn.relu
            normalizer_fn2 = tf.contrib.slim.batch_norm
            normalizer_params2 = {
                'is_training': is_training,
                'center': True,
                'scale': True,
            ...
            weights_initializer02 = tf.truncated_normal_initializer(stddev=0.09)
            weights_regularizer02 = tf.contrib.layers.l2_regularizer(0.00004)
            feature_map_2 = slim.conv2d(net,
                                        # 18,
                                        3 * (5 + self.class_num),
                                        1,
                                        stride=1,
                                        activation_fn=None,
                                        normalizer_fn=normalizer_fn2,
                                        normalizer_params=normalizer_params2,
                                        weights_initializer=weights_initializer02,
                                        weights_regularizer=weights_regularizer02,
                                        # scope='feature_map_002'
                                        )

            feature_map_2 = tf.identity(feature_map_2, name='feature_map_002')
            # inter2 = conv2d(inter2, 128, 1)

            activation_fncheck1 = tf.nn.relu6
            # activation_fncheck1 = tf.nn.relu
            normalizer_fncheck1 = tf.contrib.slim.batch_norm
            normalizer_paramscheck1 = {
                'is_training': is_training,
                'center': True,
                'scale': True,
                'decay': 0.9997,
            ...
            weights_initializercheck1 = tf.truncated_normal_initializer(stddev=0.09)
            weights_regularizercheck1 = tf.contrib.layers.l2_regularizer(0.00004)
            inter2 = slim.conv2d(inter2, 128, 1,
                                 activation_fn=activation_fncheck1,
                                 normalizer_fn=normalizer_fncheck1,
                                 normalizer_params=normalizer_paramscheck1,
                                 weights_initializer=weights_initializercheck1,
                                 weights_regularizer=weights_regularizercheck1,
                                 # scope='feature_map_check2'
                                 )

            inter2 = upsample_layer(inter2, route_1.get_shape().as_list() if self.use_static_shape else tf.shape(route_1))
            concat2 = tf.concat([inter2, route_1], axis=3)

            # _, feature_map_3 = yolo_block(concat2, 128,is_training=is_training)
            net = conv2d(concat2, channel_list["yolov3_yolov3_head_Conv_16"], 1, is_training=is_training)  # 16
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_17"], 3, is_training=is_training)  # 17
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_18"], 1, is_training=is_training)  # 18
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_19"], 3, is_training=is_training)  # 19
            net = conv2d(net, channel_list["yolov3_yolov3_head_Conv_20"], 1, is_training=is_training)  # 20
            feature_map_3 = conv2d(net, channel_list["yolov3_yolov3_head_Conv_21"], 3, is_training=is_training)  # 21

            weights_initializer03 = tf.truncated_normal_initializer(stddev=0.09)
            weights_regularizer03 = tf.contrib.layers.l2_regularizer(0.00004)
            activation_fn3 = tf.nn.relu6
            # activation_fn3 = tf.nn.relu
            normalizer_fn3 = tf.contrib.slim.batch_norm
            normalizer_params3 = {
                'is_training': is_training,
                'center': True,
                'scale': True,
            ...
            feature_map_3 = slim.conv2d(feature_map_3,
                                        # 14,
                                        3 * (5 + self.class_num),
                                        1,
                                        stride=1,
                                        activation_fn=None,
                                        normalizer_fn=normalizer_fn3,
                                        normalizer_params=normalizer_params3,
                                        weights_initializer=weights_initializer03,
                                        weights_regularizer=weights_regularizer03,
                                        # scope='feature_map_003'
                                        )
            feature_map_3 = tf.identity(feature_map_3, name='feature_map_003')
