...
### Some paths
train_file = './data/wider_train_2.txt'  # The path of the training txt file.
val_file = './data/wider_val_2.txt'  # The path of the validation txt file.
restore_path = './checkpoint/xishutrain/best_model_Epoch_14_step_43199_mAP_0.4760_loss_12.2275_lr_0.001'  # The path of the weights to restore.
save_dir = './checkpoint/xishutrain/'  # The directory of the weights to save.
...
anchor_path = './data/wider_anchors.txt'  # The path of the anchor txt file.
class_name_path = './data/wider.names'  # The path of the class names.
...
optimizer_name = 'sgd'  # Chosen from [sgd, momentum, adam, rmsprop]
...
lr_decay_epoch = 4  # Epochs after which learning rate decays. Int or float. Used when chosen `exponential` and `cosine_decay_restart` lr_type.
...
pw_boundaries = [25,80]  # epoch based boundaries
# pw_values = [learning_rate_init, 5e-3, 3e-3, 1e-3]
pw_values = [learning_rate_init, 1e-4, 1e-5]
...
update_part = ['yolov3/darknet53_body']
...
# nms
nms_threshold = 0.35  # iou threshold in nms operation
...
nms_topk = 400  # keep at most nms_topk outputs after nms
# mAP eval
